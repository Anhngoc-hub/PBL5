...
function filterTicketTable() {
    const k = document.getElementById("ticketSearchInput").value.trim().toUpperCase();
    renderTicketDataToTable(originalTicketsData.filter(t => (t.locker?.location || '').toUpperCase().includes(k) || (t.locker?.id || '').toUpperCase().includes(k)));
}

function clearTicketSearch() { document.getElementById("ticketSearchInput").value = ""; renderTicketDataToTable(originalTicketsData); }
...
async function filterSessions() {
    ...
    isSearching = (lid !== "" || document.getElementById("statusFilter").value !== "ALL");
    renderSessionTable(await apiCall(`${ENDPOINTS.SESSIONS}/search?lockerId=${encodeURIComponent(lid)}&status=${document.getElementById("statusFilter").value}&sortBy=${document.getElementById("sortField").value}`));
}
